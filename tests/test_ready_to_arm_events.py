...
async def test_ready_to_arm_modes_changed_event_with_door_sensor_multiple_modes(
    ...
    with patch_alarmo_integration_dependencies(storage):
        ...
        print(
            f"DEBUG: Sensor manager config: {hass.data[const.DOMAIN]['sensor_handler']._config}"
        )
        ...
        print(
            f"DEBUG: Ready to arm modes after sensor trigger: {alarm_entity._ready_to_arm_modes}"
        )
        print(
...
async def test_ready_to_arm_with_bypass_sensors(
    ...
    with patch_alarmo_integration_dependencies(storage):
        ...
        # Open bypass sensor - should only block armed_home (not armed_away due to bypass)
...
async def test_ready_to_arm_with_always_on_sensors(
    ...
    with patch_alarmo_integration_dependencies(storage):
        ...
        # This puts the alarm in "triggered" state with open sensors, making it not ready to arm
...
async def test_home_assistant_event_bus_ready_to_arm_event(
    ...
    with patch_alarmo_integration_dependencies(storage):
        ...
        assert len(ready_events) >= EXPECTED_SINGLE_EVENT, (
            f"Expected at least {EXPECTED_SINGLE_EVENT} HA event bus 'alarmo_ready_to_arm_modes_updated' event, "
            f"got {len(ready_events)}. This is required for the template entity example in the docs to work."
        )
