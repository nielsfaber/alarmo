...
export class AutomationEditorCard extends LitElement {
  ...
  async firstUpdated() {
    ...
    if (this.item) {
      if (automations[this.item] && !automations[this.item].is_notification)
        this.data = omit(automations[this.item], ['automation_id', 'is_notification', 'enabled']) as AlarmoAutomation;
  ...
  <settings-row .narrow=${this.narrow} .large=${true}>
    ...
    <span slot="description">${localize(
      'panels.actions.cards.new_action.fields.event.description',
      this.hass.language
    )}</span>
  ...
  <settings-row .narrow=${this.narrow}>
    ...
    <span slot="description">${localize(
      'panels.actions.cards.new_action.fields.mode.description',
      this.hass.language
    )}</span>
  ...
  <settings-row .narrow=${this.narrow} .large=${true}>
    ...
    <span slot="description">${localize(
      'panels.actions.cards.new_action.fields.entity.description',
      this.hass.language
    )}</span>
  ...
  <settings-row .narrow=${this.narrow}>
    ...
    <span slot="description">${localize(
      'panels.actions.cards.new_action.fields.action.description',
      this.hass.language
    )}</span>
    <div>
    <mwc-button
  ...
  <settings-row .narrow=${this.narrow} .large=${true}>
    ...
    <span slot="description">${localize(
      'panels.actions.cards.new_action.fields.name.description',
      this.hass.language
    )}</span>
  ...
  private updateEntities(entities: string[]) {
    ...
    entities.forEach((entity, i) => {
      if (
        i < actions.length &&
        computeDomain(actions[i].service || actions[i].service_data?.entity_id || '') == computeDomain(entity[i])
      ) {
        actions[i] = {
          ...actions[i],
          service_data: {
  ...
  private updateAction(value: string) {
    this.data = {
      ...this.data!,
      actions: this.data!.actions.map(e =>
        Object({ ...e, service: (computeDomain(e.service_data?.entity_id || '') || 'homeassistant') + '.' + value })
      ),
    };
  }
