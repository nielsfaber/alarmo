...
export class SensorsOverviewCard extends SubscribeMixin(LitElement) {
  ...
  private tableColumns(): Dictionary<TableColumn> {
    const formatId = (...input: string[]) => input.map(e => e.replace(".", "_")).join('_');
    ...
    return {
      ...
      name: {
        ...
        renderer: (data: AlarmoSensor & { name: string }) => html`
          ...
          ${data.area == noArea
            ? html`<ha-tooltip for="${formatId(data.entity_id, 'name')}">${localize('panels.sensors.cards.sensors.table.no_area_warning', this.hass.language)}</ha-tooltip>`
            : nothing
          }
          <span class="secondary ${!data.enabled ? 'disabled' : ''}" id="${formatId(data.entity_id, 'name', 'secondary')}">
            ${data.entity_id}
          </span>
          ${data.area == noArea
            ? html`<ha-tooltip for="${formatId(data.entity_id, 'name', 'secondary')}">${localize('panels.sensors.cards.sensors.table.no_area_warning', this.hass.language)}</ha-tooltip>`
            : nothing
          }
        `,
      },
      modes: {
        ...
        renderer: (data: AlarmoSensor) => html`
          ...
          ${data.area == noArea
            ? html`<ha-tooltip for="${formatId(data.entity_id, 'modes')}">${localize('panels.sensors.cards.sensors.table.no_area_warning', this.hass.language)}</ha-tooltip>`
            : nothing
          }
        `,
      },
