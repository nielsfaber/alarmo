...
class EventHandler:
    ...
    def async_handle_event(self, event: str, area_id: str, args: dict = {}):
        ...
        if event in [
            ...
            supported_modes = dict(filter(lambda el: el[1] & alarm_entity.supported_features, const.MODES_TO_SUPPORTED_FEATURES.items()))
            modes = {
                k.value: (k.value in args["modes"])
                for k in supported_modes.keys()
            }
            data = {
                "area_id": area_id,
                "entity_id": alarm_entity.entity_id,
